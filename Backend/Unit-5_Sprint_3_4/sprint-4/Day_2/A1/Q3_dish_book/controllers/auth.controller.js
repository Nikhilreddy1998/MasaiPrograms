...
const signupRoute = async(req,res)=>{
    try {
        ...
        bcrypt.hash(password,10,async(err,hash)=>{
            if(err) return res.status(500).json({error:"Something went wrong",message:err.message})
...
const loginRoute = async(req,res)=>{
    try {
        ...
        bcrypt.compare(password,hash,async(err,result)=>{
           if(result==true){
               let token = jwt.sign({userId:checkUser._id,role:checkUser.role},process.env.JWT_SECRET_KEY,{expiresIn:"1h"})
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (isTokenValid)
      return res.status(403).json({ message: "This token is blacklisted" });
    ...
    if (error.message == "jwt expired") {
      res
        ...
        .json({
          message:
            "Password reset link has been expired, please click forgot password again to reset the password",
        });
    } else {
